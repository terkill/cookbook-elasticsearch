...
class ElasticsearchCookbook::PluginProvider < Chef::Provider::LWRPBase
  ...
  action :install do
    ...
    unless plugin_exists(es_conf.path_plugins[es_install.type], name)
      if node['platfarm_family'] == 'windows'
        system("#{new_resource.bindir}/plugin install #{name}#{es_install.version}#{url}")
      else
        ...
        if !url or url == name
            shell_out!("#{es_conf.path_bin[es_install.type]}/plugin install #{name}".split(' '), user: username, group: groupname)
        else
            shell_out!("#{es_conf.path_bin[es_install.type]}/plugin --url #{url} --install #{name}".split(' '), user: username, group: groupname)
        end
      end
